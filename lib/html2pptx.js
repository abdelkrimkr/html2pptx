...
class HTML2PPTX {
    ...
    calculatePosition($, $elem, style, containerWidth, containerHeight) {
        ...
        // Get parent element and its styles
        const parent = $elem.parent();
        ...
        const flexDirection = parentStyle['flex-direction'] || 'row';
        
        if (isFlexContainer) {
            return this.calculateFlexPosition($, $elem, style, parentStyle, containerWidth, containerHeight);
        }
        
        // Handle regular flow layouts
    ...
    /**
     * Calculate position for flex layout items
     */
    calculateFlexPosition($, $elem, style, parentStyle, containerWidth, containerHeight) {
        const parent = $elem.parent();
        const siblings = parent.children().toArray();
        const index = siblings.indexOf($elem[0]);
        const totalSiblings = siblings.length;
        
        // Parse parent container properties
        const flexDirection = parentStyle['flex-direction'] || 'row';
        const gap = this.parsePosition(parentStyle.gap || '0');
        const paddingTop = this.parsePosition(parentStyle['padding-top'] || parentStyle.padding || '0');
        const paddingBottom = this.parsePosition(parentStyle['padding-bottom'] || parentStyle.padding || '0');
        const paddingLeft = this.parsePosition(parentStyle['padding-left'] || parentStyle.padding || '0');
        const paddingRight = this.parsePosition(parentStyle['padding-right'] || parentStyle.padding || '0');
        
        // Convert container dimensions from points to inches
        const containerWidthInches = containerWidth / 72;
        const containerHeightInches = containerHeight / 72;
        
        // Calculate available space
        const paddingTopInches = paddingTop / 72;
        const paddingBottomInches = paddingBottom / 72;
        const paddingLeftInches = paddingLeft / 72;
        const paddingRightInches = paddingRight / 72;
        const gapInches = gap / 72;
        
        const availableWidth = containerWidthInches - paddingLeftInches - paddingRightInches;
        const availableHeight = containerHeightInches - paddingTopInches - paddingBottomInches;
        
        // Calculate positions based on flex direction
        if (flexDirection === 'column') {
            // Vertical layout
            const totalGaps = (totalSiblings - 1) * gapInches;
            
            // Check if items have flex property
            const flex = style.flex || '0';
            const hasFlexGrow = flex !== '0' && flex !== 'none';
            
            let itemHeight;
            if (hasFlexGrow) {
                // Calculate equal height for flex items
                itemHeight = (availableHeight - totalGaps) / totalSiblings;
            } else {
                // Use specified height or default
                const specifiedHeight = this.parsePosition(style.height || '60');
                itemHeight = specifiedHeight / 72;
            }
            
            // Calculate Y position
            const yPosition = paddingTopInches + (index * (itemHeight + gapInches));
            ...
            let itemWidth;
            if (hasFlexGrow) {
                // Calculate equal width for flex items
                itemWidth = (availableWidth - totalGaps) / totalSiblings;
            } else {
                // Use specified width or default
                const specifiedWidth = this.parsePosition(style.width || '200');
                itemWidth = specifiedWidth / 72;
            }
            
            // Calculate X position
            const xPosition = paddingLeftInches + (index * (itemWidth + gapInches));
    ...
    parseTextOptions($, $elem, style) {
        ...
        if (style['border-radius']) {
            ...
            if (borderRadius > 0) {
                ...
                // Convert pixels to inches for rectRadius
                options.rectRadius = borderRadius / 72;
            }
        }
