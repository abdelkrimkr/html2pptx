...
class HTML2PPTX {
    ...
    calculateElementPosition($, $elem, style) {
        ...
        // Handle flex-specific or grid-specific dimensions if needed
        if (parentStyle.display === 'grid') {
            const gridPosition = this.calculateGridPosition($, $elem, parentStyle);
            x = gridPosition.x;
            y = gridPosition.y;
            w = gridPosition.w;
            h = gridPosition.h;
        } else if (parentStyle.display === 'flex' && parentStyle['flex-direction'] === 'column') {
     ...
     * This function is now refactored to delegate tasks to smaller helper methods.
     */
    calculateAbsolutePosition($, $elem, parentStyle, elemWidth, elemHeight) {
        let { x, y } = this.getParentPosition($, $elem, parentStyle);

        // Add vertical offset from previous siblings
        y += this.getVerticalOffset($, $elem, parentStyle);

        // Apply horizontal and vertical centering
        x = this.applyHorizontalCentering($, $elem, parentStyle, x, elemWidth);
        y = this.applyVerticalCentering($, $elem, parentStyle, y);

        return { x, y };
    }

    /**
     * Calculates the vertical offset based on previous siblings' heights and margins.
     */
    getVerticalOffset($, $elem, parentStyle) {
        let y = 0;
        const parent = $elem.parent();
        if (parent.length === 0) return y;
        ...
        for (let i = 0; i < index; i++) {
            ...
            // Add sibling height and margins
            y += this.getElementHeight($, sibling, siblingStyle);
    ...
    /**
     * Recursively gets the position of the parent element.
     */
    getParentPosition($, $elem, parentStyle) {
        let x = 0, y = 0;
        const parent = $elem.parent();
    ...
    /**
     * Applies horizontal centering if text-align is center.
     */
    applyHorizontalCentering($, $elem, parentStyle, currentX, elemWidth) {
        const currentStyle = this.getComputedStyle($, $elem[0]);
        if (currentStyle['text-align'] === 'center' || parentStyle['text-align'] === 'center') {
            ...
            return currentX - this.parsePixelValue(parentStyle.padding || '0') + (availableWidth - elemWidth) / 2;
        }
        return currentX;
    }

    /**
     * Applies vertical centering for flex containers.
     */
    applyVerticalCentering($, $elem, parentStyle, currentY) {
        const parent = $elem.parent();
        if (parent.length > 0 && parent.prop('tagName')?.toLowerCase() !== 'body') {
            const grandParent = parent.parent();
            const grandParentStyle = grandParent.length > 0 ? this.getComputedStyle($, grandParent[0]) : {};

            if (grandParentStyle.display === 'flex' && grandParentStyle['justify-content'] === 'center' && grandParentStyle['flex-direction'] === 'column') {
                ...
                parent.children().each((i, sib) => {
                    ...
                    totalContentHeight += this.getElementHeight($, $(sib), sibStyle);
    ...
    /**
     * Calculate position for an element within a CSS Grid container
     */
    calculateGridPosition($, $elem, parentStyle) {
        const parent = $elem.parent();
        const children = parent.children();
        const index = children.index($elem);

        // Get grid properties
        const gridTemplateColumns = (parentStyle['grid-template-columns'] || '1fr').split(' ');
        const gridTemplateRows = (parentStyle['grid-template-rows'] || 'auto').split(' ');
        const gap = this.parsePixelValue(parentStyle.gap || '0');
        const columnGap = this.parsePixelValue(parentStyle['column-gap'] || gap);
        const rowGap = this.parsePixelValue(parentStyle['row-gap'] || gap);

        const numColumns = gridTemplateColumns.length;
        const parentWidth = this.parsePixelValue(parentStyle.width || this.options.htmlWidth.toString());
        const parentHeight = this.parsePixelValue(parentStyle.height || this.options.htmlHeight.toString());
        const parentPadding = this.parsePixelValue(parentStyle.padding || '0');

        const availableWidth = parentWidth - (parentPadding * 2) - (columnGap * (numColumns - 1));
        
        // Calculate column widths
        const columnWidths = gridTemplateColumns.map(c => {
            if (c.endsWith('fr')) {
                const frTotal = gridTemplateColumns.reduce((total, val) => {
                    return val.endsWith('fr') ? total + parseFloat(val) : total;
                }, 0);
                return (parseFloat(c) / frTotal) * availableWidth;
            }
            return this.parsePixelValue(c);
        });
        ...
        for (let i = 0; i < rowIndex; i++) {
            const rowHeight = (gridTemplateRows.length > i && gridTemplateRows[i] !== 'auto')
                ? this.parsePixelValue(gridTemplateRows[i], parentHeight)
                : this.getElementHeight($, children.eq(i * numColumns), this.getComputedStyle($, children.eq(i * numColumns)[0]));
            y += rowHeight + rowGap;
        }

        const w = columnWidths[colIndex];
        let h = this.getElementHeight($, $elem, this.getComputedStyle($, $elem[0])); // Default height

        if(gridTemplateRows.length > rowIndex && gridTemplateRows[rowIndex] !== 'auto') {
            h = this.parsePixelValue(gridTemplateRows[rowIndex], parentHeight);
        }
    ...
    /**
     * Helper function to calculate the height of an element.
     */
    getElementHeight($, $elem, style) {
        if (style.height) {
        ...
        const fontSize = this.parseFontSize(style['font-size'] || '16px');
        const lineHeight = style['line-height'] ? this.parsePixelValue(style['line-height']) : fontSize * 1.2;
        const estimatedLines = Math.ceil(text.length / 50) || 1; // Ensure at least one line
        return lineHeight * estimatedLines + fontSize * 0.5;
    }
    ...
    parseTextOptions($, $elem, style, tagName) {
        ...
        // Hyperlink support
        if (tagName === 'a' && $elem.attr('href')) {
            options.hyperlink = { url: $elem.attr('href') };
        }
