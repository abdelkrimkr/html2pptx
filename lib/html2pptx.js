...
class HTML2PPTX {
    ...
    processImageElements($, slide, container) {
        container.find('img').each((i, elem) => {
            ...
            try {
                ...
                // PptxGenJS uses `data:` property for base64
                if (src.startsWith('data:image')) {
                    imageConfig.data = src;
                } else {
                    imageConfig.path = src;
                }
    ...
    parseTextOptions($, $elem, style, tagName) {
        const options = {
            ...
            shrinkText: true,
            margin: 0 // Removes internal padding that misaligns elements
        };
        ...
        if (style['letter-spacing']) {
            options.charSpacing = this.parsePixelValue(style['letter-spacing']) / 1.333; // Convert px to pt approx
        }

        // Extract line-height
        let hasLineSpacing = false;
        if (style['line-height']) {
            options.lineSpacing = this.parsePixelValue(style['line-height']) * 0.75; // Approx px to pt
            hasLineSpacing = true;
        }
        ...
        if (tagName === 'li') {
            ...
            if (parent.length > 0) {
                ...
                // Default to bullet if we can't find parent
                options.bullet = true;
            }
        } else if (tagName === 'ul') {
            options.bullet = true;
        ...
        // Use paraSpaceAfter instead of lineSpacing for bullets to avoid excessive gaps
        if (options.bullet && hasLineSpacing) {
            options.paraSpaceAfter = options.lineSpacing;
