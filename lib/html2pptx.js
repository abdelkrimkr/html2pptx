...
class HTML2PPTX {
    ...
    calculateElementPosition($, $elem, style) {
        ...
        // Width: try to get from style, otherwise use full slide width
        if (style.width) {
        ...
        // Height: based on font size and content
        if (style.height) {
        ...
        if (parentStyle.display === 'flex' && parentStyle['flex-direction'] === 'column') {
            ...
            for (let i = 0; i < index; i++) {
                ...
                if (siblingStyle.height) {
                ...
                } else if (siblingStyle.flex) {
                    // For flex: 1, estimate height based on available space
                    const flexCount = siblings.filter((j, el) => {
                        const s = this.getComputedStyle($, el);
                        return s.flex === '1' || s.flex === '1 1 0%';
                    }).length;
                    ...
                    // Default height based on font size
